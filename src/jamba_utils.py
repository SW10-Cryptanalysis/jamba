...
def fuse_jamba_experts(
	state_dict: dict, num_experts: int=8, moe_layers: tuple[int] = (1, 3, 5, 7),
) -> tuple[dict, int]:
	"""Fuses individual MoE weights into the single blocks."""
	new_state_dict = state_dict.copy()
	fused_count = 0

	for layer in moe_layers:
		if (
			f"model.layers.{layer}.feed_forward.experts.0.gate_proj.weight"
			not in state_dict
		):
			continue
		...
		gate_up_fused = torch.stack(
			[
				torch.cat([g, u], dim=0)
				for g, u in zip(gate_projs, up_projs, strict=False)
			],
		)
		new_state_dict[f"model.layers.{layer}.feed_forward.experts.gate_up_proj"] = (
			gate_up_fused
		)

		down_fused = torch.stack(down_projs)
		new_state_dict[f"model.layers.{layer}.feed_forward.experts.down_proj"] = (
			down_fused
		)
...
def prepare_checkpoint_for_fast_path(output_dir: str = str(cfg.output_dir)) -> None:
	"""Find the latest checkpoint on disk and fuses it if necessary."""
	...
	if os.path.exists(safetensors_path) and has_safetensors:
		logger.info(f"Inspecting safetensors checkpoint at: {latest_ckpt}")
		state_dict = load_file(safetensors_path)
		new_state_dict, fused_count = fuse_jamba_experts(
			state_dict,
		...
		if fused_count > 0:
			logger.debug(
				f"Fusing {fused_count} layers into Fast Path format "
				"and overwriting checkpoint...",
			)
		...
		logger.info(f"Inspecting PyTorch checkpoint at: {latest_ckpt}")
		...
		if fused_count > 0:
			logger.debug(
				f"Fusing {fused_count} layers into Fast Path format and "
