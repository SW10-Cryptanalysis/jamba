...
def fuse_jamba_experts(state_dict, num_experts=8, moe_layers=(1, 3, 5, 7)):
    ...
    for l in moe_layers:
        # Check if fusion is actually needed (skip if already fused)
        ...
        # Concatenate Gate and Up, then stack (Jamba SwiGLU format)
...
def prepare_checkpoint_for_fast_path(output_dir="./jamba-cipher-results"):
    ...
    # Hugging Face usually saves as safetensors, fallback to standard PyTorch bin
    ...
    if os.path.exists(safetensors_path) and has_safetensors:
        ...
        new_state_dict, fused_count = fuse_jamba_experts(state_dict)
